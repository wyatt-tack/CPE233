...
inout SDAin
     ...
     //logic for SDA in/out writing
     logic SDA;
     logic SDAW; //SDA for writing
     logic SDAR; //SDA for reading   
     logic SDAinOut;
    assign SDAin  = !SDAinOut ? SDAW : 1'bz;
    always @ (posedge clk)
    begin
       SDAR <= SDAin; 
       SDAW <= SDA;
...
//hold state for in between cycles
if(PS == holdW) 
begin
    if (holdCount == 8'h80)
    begin
...
PS<=NS;
//state evaluations for setting register logic
case(PS)
     holdW: dataR = storeData;    
    dataR7: storeData[7] = SDAR;
    dataR6: storeData[6] = SDAR;
    dataR5: storeData[5] = SDAR;
    dataR4: storeData[4] = SDAR;
    dataR3: storeData[3] = SDAR;
    dataR2: storeData[2] = SDAR;
    dataR1: storeData[1] = SDAR;
    dataR0: storeData[0] = SDAR;
endcase
end 
//-----------------------------------------------------------------------
